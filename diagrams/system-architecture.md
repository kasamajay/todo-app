...
Everything runs through Docker Compose (see [`../decisions/0001-docker-only-dev-workflow.md`](../decisions/0001-docker-only-dev-workflow.md)): a `web` container running the Vite dev server and an `api` container running the Go backend under `air`, joined by Docker's default bridge network. The browser only ever talks to the `web` container; the `api` container is never reached directly from outside Docker in normal use.
...
flowchart LR
    ...
    subgraph Compose["Docker Compose network"]
        ...
        subgraph ApiC["api container (golang:1.22)"]
            Air["air (hot reload)"]
            Go["Go API\n:8080"]
        end
    ...
    Air -. "rebuilds + restarts on .go change\n(poll-based, see decisions/0008)" .-> Go
