...
- **Production mode** (`docker compose -f docker-compose.prod.yml up -d --build`, port 8081): everything is compiled and bundled **once, at image build time**. **nginx** serves the resulting static files and reverse-proxies `/api/*` to the Go API. No Node process runs at all.
...
| Go API | `air` rebuilds + restarts on `.go` changes | Static binary, `alpine`, not published on the host |
...
`fetch('/api/boards')` → Vite on `:5173` → `http://api:8080/api/boards` over the Compose network (Docker service-name DNS). The Go side has its own reload loop, but no browser push: `air` watches `.go` files, rebuilds and restarts the binary, and the next API call simply hits the new process.
...
sequenceDiagram
    ...
    participant A as Go api :8080<br/>(Compose network only)
...
| Uploads | `client_max_body_size 11m` | nginx's default is 1 MB, which would reject the API's 10 MB attachments with 413 before they reach Go. |
| JS/CSS/JSON | `gzip on` | The 177 kB bundle goes over the wire as 55 kB. |

The Go API has **no host port** in `docker-compose.prod.yml` (`curl localhost:8080` fails). nginx is the only way in, which matches what the browser saw in dev, where it only ever talked to `:5173`.
...
curl -i http://localhost:8081/api/boards     # 401 from the Go api, via nginx
