...
## Key files
- `api/internal/handlers/labels_handler.go` — CRUD + the delete-strips-references cascade.
- `api/internal/handlers/tasks_handler.go` — `label_ids` validation in `Create`/`Update` (`validateLabelIDs`).
- `api/internal/handlers/boards_handler.go` — board-delete cascade now also removes the board's labels.
- `api/internal/storage/labels_store.go` — `ListByBoard`, `ListByBoardAny`.
- `api/internal/models/models.go` — `Label`, `Task.LabelIDs`, `LabelColors`, `IsValidLabelColor`.
