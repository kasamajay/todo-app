...
## What it does
- **Bootstrap**: on first API startup, if the user store is empty, `internal/bootstrap.BootstrapAdmin` creates `admin@todo.io` with a securely random generated password (`crypto/rand`, 18 bytes, base64url-encoded) and prints the plaintext to the server log exactly once — it's never stored or shown again. Subsequent restarts are no-ops (the store already has users).
...
## Key files
- `api/internal/handlers/admin_handler.go`
- `api/internal/bootstrap/bootstrap.go` — first-run admin creation
- `api/internal/middleware/middleware.go` — `RequireAdmin`
